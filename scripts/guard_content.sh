...
# - Falls back to a full scan of the content dir when no diff is available
...
# Only the blog collection: its layout (BlogLayout) emits the title as the page
# H1, so a body "# " heading is a true double-H1. The pages/ collection layout
# does NOT emit an H1, so its body "# " is legitimate and must not be gated.
...
if [ "${#targets[@]}" -eq 0 ]; then
  echo "content guard: no changed .md/.mdx — full scan of $CONTENT_DIR"
